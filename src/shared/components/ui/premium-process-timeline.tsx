...
const DEMO_STEPS: ProcessStep[] = [
  {
    ...
    details: ["Instant access", "No credit card required", "Sign in with Google", "Guest mode available"],
    duration: "30 seconds",
  ...
  },
  {
    ...
    description: "Enable only the modules you need. Set up your first task, log a habit, or connect your budget.",
    details: ["Toggle modules instantly", "Clean distractions", "Guided onboarding", "Flexible setup"],
    duration: "2 minutes",
  ...
  },
  {
    ...
    details: ["Cross-module data flow", "Actionable insights", "Zero double-entry", "Smart notifications"],
    duration: "Ongoing",
...
const TimelineNav = ({ steps, activeStep, onStepClick }: { steps: ProcessStep[], activeStep: string, onStepClick: (id: string) => void }) => (
  <div className="flex flex-col md:flex-row items-center justify-between gap-6 max-w-5xl mx-auto">
    <div className="flex items-center gap-4">
      ...
      <span className="text-xl font-bold text-text-primary font-display tracking-tight">How It Works</span>
    </div>
...
const TimelineContent = ({ step }: { step: ProcessStep }) => (
  <div className="flex flex-col justify-center">
    <span className="text-sm font-bold text-brand uppercase tracking-wider">{step.id}</span>
    <h2 className="text-[32px] font-bold mt-2 text-text-primary font-display leading-tight">{step.title}</h2>
    <p className="mt-2 text-text-muted font-medium text-[15px]">{step.subtitle}</p>
    <p className="mt-5 text-text-secondary leading-relaxed text-[16px]">{step.description}</p>
    ...
    <div className="mt-8 grid sm:grid-cols-2 gap-4">
      {step.details.map((detail, i) => (
        <div key={i} className="flex items-start gap-3">
          ...
          <span className="text-[14px] text-text-secondary leading-snug">{detail}</span>
        </div>
      ))}
    </div>
    ...
    <div className="mt-8 flex items-center gap-3 p-4 bg-bg-subtle border border-border-subtle rounded-xl w-fit">
      ...
      <span className="text-[14px] font-semibold text-text-primary">Time to complete: {step.duration}</span>
    </div>
  </div>
);
    ...
    <div className="flex items-center justify-center">
        <div className="w-[300px] h-[600px] bg-bg-card rounded-[40px] p-4 border border-border-medium shadow-[0_20px_40px_rgba(0,0,0,0.12)] dark:shadow-[0_20px_40px_rgba(0,0,0,0.4)] relative">
            ...
            <div className="w-full h-full bg-bg-base rounded-[24px] overflow-hidden">
                <img src={image} alt="App Screenshot" className="w-full h-full object-cover transition-opacity duration-300" />
            </div>
        </div>
    </div>
);
...
const BottomTimeline = ({ steps, activeIndex, onStepClick }: { steps: ProcessStep[], activeIndex: number, onStepClick: (id: string) => void }) => (
  <div className="mt-16 sm:mt-24 px-4 sm:px-0">
    ...
    <div className="mt-6 flex justify-between">
      {steps.map((step, i) => (
        <button key={step.id} onClick={() => onStepClick(step.id)} className="text-center w-1/4 group flex flex-col items-center">
          <span className={cn(
             "text-[14px] font-bold transition-colors font-display tracking-wide",
             i <= activeIndex ? "text-brand" : "text-text-muted group-hover:text-text-secondary"
          )}>
            STEP {step.id}
          </span>
          <p className={cn(
             "text-[13px] mt-1.5 transition-colors font-medium hidden sm:block",
             i <= activeIndex ? "text-text-primary" : "text-text-muted group-hover:text-text-secondary"
          )}>
            {step.title}
          </p>
        </button>
